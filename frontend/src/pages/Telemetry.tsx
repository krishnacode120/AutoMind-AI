  ...
  return (
    <div className="resource-page">
      <PageHeader
        ...
        subtitle={
          vehicle
            ? `Live reading for ${vehicle.name}`
            : "Vehicle sensor readings"
        }
      />
      {(vehiclesLoading || isLoading) && <Loading />}
      {vehicleId && <SimulateDrive key={vehicleId} vehicleId={vehicleId} />}
      {!vehiclesLoading && !vehicle && (
        <Card className="resource-card">
          <p className="resource-empty">Add a vehicle to view telemetry.</p>
        </Card>
      )}
      {isError && (
        <Card className="resource-card">
          <p className="resource-empty">Telemetry is unavailable.</p>
        </Card>
      )}
      {vehicle && !isLoading && !isError && !telemetry && (
        <p className="resource-empty">
          No telemetry has been recorded for this vehicle.
        </p>
      )}
      {telemetry && (
        <>
          <Card className="resource-card">
            <div className="detail-grid">
              <Metric
                label="Speed"
                value={`${telemetry.speed.toFixed(1)} km/h`}
              />
              ...
              <Metric
                label="Temperature"
                value={`${telemetry.engine_temperature.toFixed(1)} C`}
              />
              <Metric
                label="Odometer"
                value={`${telemetry.odometer.toLocaleString()} km`}
              />
            </div>
          </Card>
