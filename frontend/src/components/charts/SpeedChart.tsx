  ...
  return (
    <Card className="dashboard-card">
      <div className="dashboard-card__header" style={{ marginBottom: "16px" }}>
        <h2 className="dashboard-card__title">Speed History</h2>
      </div>
      <div style={{ width: "100%", height: 300 }}>
        <ResponsiveContainer>
          <LineChart data={formattedData} margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
            ...
            <YAxis stroke="var(--color-muted)" fontSize={12} unit=" km/h" />
            <Tooltip 
              contentStyle={{ backgroundColor: "var(--color-panel-soft)", border: "1px solid var(--color-border)", borderRadius: "8px" }}
              itemStyle={{ color: "var(--color-text)" }}
            />
            <Legend />
            <Line type="monotone" dataKey="speed" name="Speed (km/h)" stroke="var(--color-primary)" strokeWidth={2} dot={false} activeDot={{ r: 6 }} />
          </LineChart>
        </ResponsiveContainer>
      </div>
    </Card>
